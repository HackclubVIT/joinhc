...
INSERT INTO
    public.profiles (id, role, email, full_name, register_no)
VALUES
    (
        NEW.id,
...
CREATE
OR REPLACE FUNCTION "public"."get_user_email"("user_id" "uuid" DEFAULT "auth"."uid"()) RETURNS "text" LANGUAGE "sql" STABLE SECURITY DEFINER
SET
...
CREATE
OR REPLACE FUNCTION "public"."handle_role_change"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ BEGIN -- If role changed to recruiter, ensure they have a record in recruiter_departments
    IF NEW.role = 'recruiter'
    AND (
        OLD.role IS NULL
        OR OLD.role != 'recruiter'
    ) THEN
INSERT INTO
    recruiter_departments (recruiter_id, department_id)
VALUES
    (NEW.id, NULL);
...
CREATE
OR REPLACE FUNCTION "public"."is_recruiter"("user_id" "uuid" DEFAULT "auth"."uid"()) RETURNS boolean LANGUAGE "sql" STABLE SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $
SELECT
    EXISTS (
...
CREATE
OR REPLACE FUNCTION "public"."update_user_role"("user_email" "text", "new_role" "text") RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE success BOOLEAN;
...
CREATE
OR REPLACE FUNCTION "public"."update_user_role"("user_id" "uuid", "new_role" "text") RETURNS boolean LANGUAGE "plpgsql" SECURITY DEFINER
SET
    "search_path" TO 'public' AS $ $ DECLARE success BOOLEAN;
...
ALTER TABLE
    "public"."application_settings" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."applications" (
    ...
    CONSTRAINT "applications_status_check" CHECK (
        (
            "status" = ANY (
                ARRAY ['pending'::"text", 'shortlisted'::"text", 'waitlisted'::"text", 'rejected'::"text"]
            )
        )
    )
);
...
ALTER TABLE
    "public"."applications" OWNER TO "postgres";
...
ALTER TABLE
    "public"."departments" OWNER TO "postgres";
...
ALTER TABLE
    "public"."recruiter_departments" OWNER TO "postgres";

ALTER TABLE
    ONLY "public"."application_settings"
ADD
...
ALTER TABLE
    ONLY "public"."recruiter_departments"
ADD
...
ALTER TABLE
    ONLY "public"."recruiter_departments"
ADD
    CONSTRAINT "recruiter_departments_recruiter_id_department_id_key" UNIQUE ("recruiter_id", "department_id");

CREATE INDEX "profiles_email_idx" ON "public"."profiles" USING "btree" ("email");

CREATE
OR REPLACE TRIGGER "on_profile_role_change"
AFTER
UPDATE
    OF "role" ON "public"."profiles" FOR EACH ROW
    WHEN (("new"."role" <> "old"."role")) EXECUTE FUNCTION "public"."handle_role_change"();

ALTER TABLE
    ONLY "public"."applications"
ADD
    CONSTRAINT "applications_applicant_id_fkey" FOREIGN KEY ("applicant_id") REFERENCES "auth"."users"("id");

ALTER TABLE
    ONLY "public"."applications"
ADD
    CONSTRAINT "applications_first_pref_dept_id_fkey" FOREIGN KEY ("first_pref_dept_id") REFERENCES "public"."departments"("id");

ALTER TABLE
    ONLY "public"."applications"
ADD
    CONSTRAINT "applications_second_pref_dept_id_fkey" FOREIGN KEY ("second_pref_dept_id") REFERENCES "public"."departments"("id");
...
ALTER TABLE
    ONLY "public"."recruiter_departments"
ADD
    CONSTRAINT "recruiter_departments_department_id_fkey" FOREIGN KEY ("department_id") REFERENCES "public"."departments"("id");

ALTER TABLE
    ONLY "public"."recruiter_departments"
ADD
    CONSTRAINT "recruiter_departments_recruiter_id_fkey" FOREIGN KEY ("recruiter_id") REFERENCES "auth"."users"("id");

ALTER TABLE
    "public"."application_settings" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "application_settings_modify_recruiter" ON "public"."application_settings" USING ("public"."is_recruiter"());

CREATE POLICY "application_settings_select_all" ON "public"."application_settings" FOR
SELECT
    USING (true);

ALTER TABLE
    "public"."applications" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "applications_select_recruiter" ON "public"."applications" FOR
SELECT
...
CREATE POLICY "applications_update_recruiter" ON "public"."applications" FOR
UPDATE
    USING ("public"."is_recruiter"());
...
CREATE POLICY "departments_select_all" ON "public"."departments" FOR
SELECT
    USING (true);
...
CREATE POLICY "profiles_select_recruiter" ON "public"."profiles" FOR
SELECT
    USING ("public"."is_recruiter"());
...
ALTER TABLE
    "public"."recruiter_departments" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "recruiters_can_manage_departments" ON "public"."recruiter_departments" USING ("public"."is_recruiter"());

CREATE POLICY "recruiters_can_view_own_departments" ON "public"."recruiter_departments" FOR
SELECT
    USING (
        (
...
,
    DELETE,
UPDATE
    ON TABLE "public"."recruiter_departments" TO "anon";
